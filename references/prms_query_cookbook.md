...
7. **Open reporting phase = provisional data (added 2026-09-07).** Snapshots newer than June 2026 carry the 2026 cycle while it is still open (`version` 8 "Reporting 2026" / 9 "IPSR 2026"). *Open* = `status = 1` **and** `end_date` after the snapshot's data date — `status` alone misfires (the June-2026 snapshot still flagged the finished 2025 phases). Default scope is the closed phases: the latest-phase chains exclude open phases by construction, but any **naive / unfiltered** count must add `version_id NOT IN (<open phase ids>)` (`NOT IN (8, 9)` on the 2026-09-07 snapshot; the system prompt states the current ids). Answer 2026 questions only when asked, and label every such figure *provisional — open reporting phase, data as of <snapshot data date>*. Always quote the snapshot date printed in the `prms_query` footer.
