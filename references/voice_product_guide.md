...
Each call has a maximum length shown in the panel before starting (currently ten minutes); the user can start again afterwards. One voice connection per user at a time and a small shared number of concurrent connections; if all are in use or the start limit is reached, the app says so and the user can try again later. If the voice service itself is temporarily unavailable, the voice button is disabled with that message and the rest of the application keeps working.
