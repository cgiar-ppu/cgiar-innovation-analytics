...
Newer snapshots carry the **2026 reporting cycle while it is still open**: `version` rows 8 "Reporting 2026" and 9 "IPSR 2026" (`status = 1`, `end_date` 2026-12-31). **Definition of open:** `status = 1` **and** `end_date` later than the snapshot's data date. `status` alone is NOT enough — the June-2026 snapshot still carried `status = 1` on the finished 2025 phases (6, 7; ended 2025-12-31, figures final), and treating them as open would have hidden 2025 entirely. `GET /api/prms/snapshot` lists the open phases of the snapshot in use. On the 2026-09-07 snapshot they hold 144 active rows (21 type-7 Innovation Developments already `status_id = 2`, 12 type-10 packages, 90 type-11 complementary innovations, the rest in QA). These numbers grow week by week and are **provisional**.

Rules:
1. **Default scope stays 2022–2025.** Per-year defaults, the all-years headline (1,852) and every Section-1 figure exclude open phases. The latest-phase chains `(1,3,4,6)` / `(2,5,7)` do this by construction; a **naive unfiltered count does not** — add `version_id NOT IN (<open phase ids>)` — on the 2026-09-07 snapshot `NOT IN (8, 9)`; derive the ids with `SELECT id FROM version WHERE status = 1 AND substr(end_date,1,10) > '<snapshot data date>'`, never hard-code them.
