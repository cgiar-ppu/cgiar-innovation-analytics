 ...
 *
 * Deployment-configured sign-in. DEV is CGIAR SSO only; other deployments can
 * retain the existing password surface when their backend explicitly allows it.
 *
...
export default function LoginScreen() {
  ...
  useEffect(() => {
    let cancelled = false
    fetch('/api/config')
      .then((res) => { if (!res.ok) throw new Error('Config unavailable'); return res.json() })
      .then((data) => {
        if (!cancelled && data) {
          setSelfSignupEnabled(Boolean(data.self_signup))
          setSsoEnabled(Boolean(data.sso_enabled))
          setPasswordEnabled(data.password_login_enabled !== false)
          setConfigReady(true)
          setAllowedDomains(
      ...
      .catch(() => { if (!cancelled) setError('Could not load sign-in settings. Please refresh and try again.') })
    return () => {
      cancelled = true
    }
  }, [])
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-4 bg-[var(--bg)]">
      ...
      >
        ...
        {!configReady && !error && <p className="text-sm text-[var(--text-muted)] text-center">Loading sign-in…</p>}
        {!passwordEnabled && error && <p role="alert" className="text-xs text-red-500">{error}</p>}
        {passwordEnabled && <form onSubmit={onSubmit} className="space-y-3">
          {mode === 'signup' && (
            <input
              type="text"
              autoComplete="name"
        ...
        {selfSignupEnabled && passwordEnabled && (
          <button
            type="button"
        ...
        {configReady && <p className="text-[10px] text-[var(--text-muted)] text-center mt-5 leading-relaxed">
          {ssoEnabled
            ? 'Use your CGIAR Microsoft account. Signing out of this tool does not sign you out of Microsoft. AI outputs require human quality assurance.'
            : mode === 'signup'
