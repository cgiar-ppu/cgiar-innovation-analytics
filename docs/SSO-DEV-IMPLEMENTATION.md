...
An existing password account must prove its application password once to link the verified issuer/subject. Existing owner ID, name and role are preserved. New SSO users receive an opaque sso: UUID owner ID and researcher role; an external role claim cannot grant admin. Changing the upstream email does not replace the stable owner ID. No historical chat rows are re-keyed.
...
Logout deletes the server session and browser cookies and requests Cognito refresh-token revocation and hosted logout. A previously issued app bearer token can remain valid for up to five minutes. An already-open WebSocket follows the app's existing connection lifecycle. Logging out of this app does not terminate the upstream Microsoft session. The existing application password path remains available; interim password self-signup is turned off in DEV.
...
Before deployment: local SSO/auth/scoping/signup/export dependency tests 68 passed, frontend auth/login/SSO tests 22 passed, production frontend build passed, full server import passed, workflow YAML parsed and diff whitespace checked. Existing infrastructure template SHA matches deployed source. Actual deployed-login results are recorded in the run report outside this repository.

Rollback triggers: legacy login unavailable, session/ownership comparison fails, SSO grants an incorrect role or private data, or unrecoverable auth errors. Prefer disabling IA_SSO_ENABLED through the reviewed DEV deployment lane while keeping legacy login. If necessary redeploy previous validated app source 3b60022a55057db5a58ef6508f6c3134b52867e7 through DEV Actions. Preserve existing volumes, Cognito pool, registration and identity tables; do not restore/delete user data as part of an auth rollback. Recheck counts and ownership afterward.
